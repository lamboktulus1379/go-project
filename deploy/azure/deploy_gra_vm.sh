...
ssh -i "$VM_SSH_KEY" -o StrictHostKeyChecking=no "${VM_USER}@${VM_HOST}" bash -lc "
set -eo pipefail
...
# Carry over YouTube env values from the local machine if provided
...
EOF
  ...
  # Ensure YouTube variables exist; append missing ones without touching DB and other app envs
